...
async function encode(payload: JwtPayload): Promise<string> {
  ...
  // @ts-ignore
  return promisify(sign)({ ...payload }, process.env.SECRET_KEY, { algorithm: 'HS256' });
}
...
async function validate(token: string): Promise<JwtPayload> {
  ...
  try {
    // @ts-ignore
    return (await promisify(verify)(token, process.env.SECRET_KEY)) as JwtPayload;
  } catch (e: any) {
    console.debug(e);
...
async function decode(token: string): Promise<JwtPayload> {
  ...
  try {
    // @ts-ignore
    return (await promisify(verify)(token, process.env.SECRET_KEY, {
